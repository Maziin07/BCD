...
FROM
    tb_pedido_roupa
        JOIN
    tb_roupa ON tb_roupa.id_roupa = tb_pedido_roupa.ROUPA_id_roupa
        JOIN
    tb_cor ON tb_cor.id_cor = tb_roupa.COR_id_cor
        JOIN
    tb_cliente ON tb_cliente.id_cliente = tb_pedido_roupa.CLIENTE_id_cliente
WHERE
    tb_cliente.COR_id_cor_favorita = tb_roupa.COR_id_cor
...
FROM
    tb_cliente cl 
        JOIN
    tb_cor c ON c.id_cor = cl.COR_id_cor_favorita
        LEFT JOIN
    tb_pedido_roupa pd ON pd.CLIENTE_id_cliente = cl.id_cliente
WHERE
    pd.CLIENTE_id_cliente IS NULL;
...
FROM
    tb_categoria c1
        LEFT JOIN
    tb_categoria c2 ON cl_id_categoria = c2.CATEGORIA_id_categoria_pai
WHERE
    cl_CATEGORIA_id_categoria_pai IS NULL;
    
	-- 4: Crie uma view chamada vw_detalhes_pedidos que exibe o nome completo do cliente (como nome_completo), o nome da roupa, a cor da roupa, a quantidade e a data do pedido.

	CREATE VIEW vw_detalhes_pedido AS
    SELECT c1_priemiro_nome,
    c2_Sobrenome,
    B_nome, tb_cor_nome,
    pd_quantidade,
    pd_data_pedido
